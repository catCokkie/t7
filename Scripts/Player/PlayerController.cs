...
namespace SilentTestimony.Player
{
	...
	public partial class PlayerController : CharacterBody2D
	{
		...
		[Export] private double _walkNoiseInterval = 0.6; // 行走时，每0.6秒发一次噪音
		[Export] private double _runNoiseInterval = 0.25; // 奔跑时，每0.25秒发一次噪音
		...
		/// <summary>
		/// 当 NoiseTimer 倒计时结束时，此方法会被自动调用。
		/// </summary>
		private void OnNoiseTimerTimeout()
		{
			// GD.Print($"发出噪音! 状态: {CurrentState}"); // 调试用
